...
namespace Pokemon.Area
{
    public class World
    {
        ...
        public World()
        {
            ...
            Terrain terrainStart = new Terrain(startID, AreaSettings.TerrainWidth, AreaSettings.TerrainHeigth, gateLocations);
        ...
        public Terrain MoveToNextTerrain(int[] vector, int[] currentTerrainID)
        {
            ...
            if (index == -1) // not found
            {
                ...
                Terrain terrain = new Terrain(nextTerrainID, AreaSettings.TerrainWidth, AreaSettings.TerrainHeigth, adjacentGateLocations);
        ...
        public string ShowMap(int[] playerLocation)
        {
            ...
            FullMap = FullMap.Remove(FullMap.IndexOf(AreaSettings.Person) + 1, 1);
            return FullMap;
        }
            ...
            for (int y = top; y < bottom; y++)
            {
                for (int x = left; x < right; x++)
                {
                    ...
                    {   // ┌ ┐└ ┘ ─ │  map icons
                        ...
                        mapPiece += $"{(CheckTwoIntArraysEqual(Terrains[id].GateLocations[2], zeroZero) ? "│" : " ")}{((y == playerLocation[0] && x == playerLocation[1]) ? $" {AreaSettings.Person} ": "   ")}{(CheckTwoIntArraysEqual(Terrains[id].GateLocations[3], zeroZero) ? "│" : " ")}\n";
